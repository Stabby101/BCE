/*
 * DIRECTIVE-HARDEN-6 Part B — pins the shipped token-redaction helpers (roster-redact.ts, HARDEN-5b). anonId is
 * a stable, prefixed, non-reversible handle; redactLobby/redactClaims replace OTHER players' token/holderToken
 * with it while leaving the recipient's OWN entry (and a blank holderToken) untouched — the property that keeps
 * the client's self-identity check working with no client change.
 */
