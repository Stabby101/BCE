...
import { cbtCanDealDamage, cbtHasArtillery, cbtHasAutocannon, cbtHasIndirectFireWeapon, cbtIsWeaponComp, type FormationUnitFacts } from './formation-unit-facts.util';
...
export const FORMATION_PREDICATES: Readonly<Record<FormationPredicateId, FormationPredicate>> = {
    ...
    'anvil-weapon': (facts, gameSystem) => gameSystem === GameSystem.ALPHA_STRIKE
        ...
        : cbtHasAutocannon(facts.unit)
            ...
            // REBASE-1 P1 (e) ruling #4 RE-HOME: gate the slice-fallback NAME match on the comp being a WEAPON
            // (E/M/B/A) — an ungated match flips on "MML 5/LRM Ammo" (X) rows. See cbtIsWeaponComp.
            || facts.unit.comp?.some(component => cbtIsWeaponComp(component.t) && component.n?.includes('LRM')) === true
            || facts.unit.comp?.some(component => cbtIsWeaponComp(component.t) && component.n?.includes('SRM')) === true,
