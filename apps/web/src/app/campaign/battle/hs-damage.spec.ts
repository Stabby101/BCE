/*
 * DIRECTIVE-PD3 P1 (PD3-12) — the ONE HS damage truth, pinned fast (the "faster spec" rule): a reconcile envelope, a
 * destroyed flag, and a tabletop level each count; a pristine unit and an `In repair` condition alone do not.
 */
...
describe('hsDamaged — the one damage truth (PD3-12)', () => {
    ...
    it('an empty envelope (heat-only, the D-030 live-only mirror) does not', () => { expect(hsDamaged(unit({ damage: env({}) }))).toBeFalse(); });
