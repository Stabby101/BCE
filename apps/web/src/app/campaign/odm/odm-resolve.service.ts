/*
 * FORKED FROM campaign/dashboard/resolve.service.ts @ 6cc5dae - DIRECTIVE-ODM-1 Phase 1 (a DRIFT SURFACE).
...
export class OdmResolveService {
    ...
    private readonly activeSalvagePct = computed(() => 0); // ODM-1 FORK: HS salvage % never applies (HS not hosted)
    ...
    readonly salvageEstimate = computed(() => {
        ...
        void tier; return 0; // ODM-1 FORK: the HS SP salvage estimate never applies
    });
    ...
    readonly combatPayPreview = computed(() => 0); // ODM-1 FORK: HS SP combat pay never applies (the IRON RULE pays in salvage)
    ...
    async confirmResolve(): Promise<void> {
        ...
        // ── ODM-3 — record the TRUE 4-tier + GM flags on the snapshot, then enforce the authored tree:
        ...
        if (node && treeData) {
            ...
            // ODM-5 — BOOK the operation's real time cost at resolve (transit + surface + the Edge's recharge;
            // the D-099 advanceDays path — subscribers ride, the pack-gated contract ticks stay inert). The
