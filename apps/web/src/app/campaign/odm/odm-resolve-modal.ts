...
@Component({
    selector: 'bce-odm-resolve-modal',
                ...
                @if (res.deployedCount()) {
                    ...
                    @if (res.odmNodeDef(); as node) {
                        ...
                        <div class="odm-tiers" data-testid="odm-tiers" role="group" aria-label="ODM outcome tier">
                            @for (t of res.ODM_TIERS; track t.id) {
                                <button type="button" class="amx-b odm-tier" [class.sel]="res.odmTier() === t.id" [attr.aria-pressed]="res.odmTier() === t.id" (click)="res.setOdmTier(t.id)">{{ t.label }}</button>
                            }
                        </div>
                        @if (node.resolveFlags.length) {
                            <div class="odm-flags" data-testid="odm-flags">
                                @for (f of node.resolveFlags; track f.id) {
                                    <label class="odm-flag"><input type="checkbox" [checked]="res.odmFlags().has(f.id)" (change)="res.toggleOdmFlag(f.id)" /> {{ f.label }}</label>
                                }
                            </div>
                            <div class="gate-note">These flags gate follow-on operations — mark what actually happened on the table.</div>
                        }
                    }
    ...
    styles: [`
        ...
        * { box-sizing: border-box; }
        ...
        .odm-flags { display: flex; flex-direction: column; gap: 5px; margin: 4px 0 8px; }
        .odm-flag { display: flex; gap: 8px; align-items: flex-start; font-size: 12.5px; color: var(--ink2); cursor: pointer; }
...
export class OdmResolveModalComponent {
