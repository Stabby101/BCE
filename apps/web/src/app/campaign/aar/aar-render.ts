...
export interface AarSection {
    id: 'command' | 'intelligence' | 'armorer' | 'personnel' | 'ledger';
    title: string;
...
export function buildAarDocument(branch: MissionBranch, ctx: AarContext): AarDocument {
    ...
    if (a?.objectiveMarks?.length) {
        ...
        if (!a?.objectives) cmd.note = 'Objective texts were not snapshotted on this resolution (resolved pre-) — results shown from the questionnaire record only.';
    } else {
    ...
    if (!a) intel.note = 'No intelligence snapshot on this resolution (resolved pre-) — assessment limited to the questionnaire record.';
        ...
        if (blu.length) {
            ...
            for (const w of blu) {
                ...
                if (h.bill?.length) {
                    ...
                    blocks.push({ p: `${h.label} — restored to service ${formatDate(h.date)} (${h.laborHours} h, ${fmt(h.cost)} C-bills, Bay ${h.bayName}; itemized bill not on record — completed pre-).` });
                }
            }
        }
    ...
    const pers: AarSection = { id: 'personnel', title: ctx.isHotspots ? 'SECTION 4 — LOSSES & PERSONNEL' : 'SECTION 4 — PERSONNEL', author: ctx.voices.personnel ?? ctx.voices.command, paragraphs: [] };
    if (ctx.isHotspots) {
