...
> from the original fork point `1398d5ea` (2026-03-30) onto **`a882d9f6`** (origin/main,
...
(2026-03-30), taken in **There is no shared git history** — no remote, no submodule, no
...
``, …) — search the file for it. This ledger covers what inline markers cannot: the **JSON and
...
| binary | 5 | favicon, 3 PWA icons, logo — BCE branding |
...
| what | why |
|---|---|
| `"serviceWorker": false` | BCE ships **no** service worker. A stale SW was the session-freshness wedge; it is also actively unregistered at boot (`index.html`, `shared/app-reset.ts`). Re-enabling it reintroduces |
| bundle budget `maximumError: 5.25MB` | raised for the campaign feature set (P3 pushed it past 5 MB). The honest fix is lazy-loading; that is a flagged follow-up, and the budget is not to be raised a third time without it. |
| component-style budget `maximumError: 22kB` | the resolve/settlement modal breached the default on tablet. |
...
| eslint/TS-eslint devDeps | stood up `nx lint web` (advisory-with-teeth; baseline 0 errors / 660 warnings). Upstream had no lint target. |
...
|---|---|---|
| `src/app/services/data.service.ts` | `DEPLOY-005`, `/012`, `SLICE-1` | era slices (`ensureSlice`), the comp-flag side-car (`hydrateSliceEq`, `compEquipment`), optional-store handling |
| `src/styles.scss` | `` | scoped-global campaign styles, hoisted out of oversized component sheets |
| `src/index.html` | `` | boot fallback + watchdog, SW unregistration, engine-URL guard |
| `src/app/app.routes.ts` | `COMPLIANCE-1` | every BCE route (`/campaign`, `/player`, `/legal`, the cover) |
| `src/app/app.config.ts` | `DEPLOY-002 P4`, `` | auth interceptor, app initializers, **service worker removed** |
...
| `src/app/models/as-force-unit.model.ts` | `` | AS Piece-Value falls back to the slim slice's folded `pv` |
...
| `src/app/components/sidebar/sidebar.component.html` | `` | BCE badge |
| `src/app/app.html` | `` | BCE lockup |
| `src/app/app.ts` | `DEPLOY-002 P4` | selector moved to `mekbay-app`; `app-root` belongs to the gated shell |
| `src/app/models/cbt-force-unit-state.model.ts` | `REBASE-1 P1` (ORDER-9 ruling #7) | `update()` guards the unguarded `data.crew.map` — a crewless/malformed live state leaves crew untouched instead of throwing inside the loader. NEW P1 fork-edit (upstream did not fix it at the pin). Witness: `battle-force.service.spec.ts` + the E2E broken-sheet proofs. |
